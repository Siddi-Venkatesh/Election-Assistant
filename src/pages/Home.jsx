...
import { trackStepClicked } from '../services/analytics';
...
export default function Home() {
  ...
  const steps = [
    {
      id: 1, title: t('step_1_title'), icon: <User className="w-6 h-6" />,
      content: t('step_1_desc'), action: t('step_1_action'), route: '/eligibility'
    },
    {
      id: 2, title: t('step_2_title'), icon: <CheckCircle className="w-6 h-6" />,
      content: t('step_2_desc'), action: t('step_2_action'), route: '/register'
    },
    {
      id: 3, title: t('step_3_title'), icon: <MapPin className="w-6 h-6" />,
      content: t('step_3_desc'), action: t('step_3_action'), route: '/booth'
    },
    {
      id: 4, title: t('step_4_title'), icon: <Calendar className="w-6 h-6" />,
      content: t('step_4_desc'), action: t('step_4_action'), route: '/guidelines'
    },
    {
      id: 5,
  ...
  const handleStepClick = (step) => {
    setActiveStep(step);
    trackStepClicked(step.id, step.title);
  };
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-16 items-center relative z-10 py-8">
      ...
      >
        ...
        <div className="flex gap-4">
          <button 
            ...
            aria-label="Start the voter registration guide"
          >
            {t('start_guide')}
          </button>
        </div>
      ...
      <div className="relative min-h-[450px]" role="region" aria-label="Election process steps">
          ...
          {!activeStep ? (
            ...
            >
              {steps.map((step, index) => (
                ...
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-indigo-50/50 to-purple-50/50 dark:from-indigo-900/10 dark:to-purple-900/10 opacity-0 group-hover:opacity-100 transition-opacity" aria-hidden="true" />
                  
                  <div className="flex items-center gap-5 relative z-10">
                    <div className="bg-slate-50 dark:bg-slate-900/50 text-slate-400 dark:text-slate-500 p-3.5 rounded-2xl group-hover:bg-gradient-to-br group-hover:from-indigo-600 group-hover:to-purple-600 group-hover:text-white transition-all duration-300 shadow-sm group-hover:shadow-md" aria-hidden="true">
                      {step.icon}
                    </div>
                    <div>
                  ...
                  <div className="w-10 h-10 rounded-full bg-slate-50 dark:bg-slate-700/50 flex items-center justify-center group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/30 transition-colors relative z-10" aria-hidden="true">
            ...
            <motion.div 
              ...
              role="region"
              aria-label={`Details for step: ${activeStep.title}`}
            >
              <button 
                ...
                aria-label="Go back to timeline overview"
              >
                <ArrowLeft className="w-4 h-4" aria-hidden="true" /> {t('back_to_timeline')}
              </button>
              
              <div className="flex flex-col items-center justify-center text-center mb-6 gap-4">
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/30 dark:to-purple-900/30 border border-indigo-100 dark:border-indigo-800/30 text-indigo-600 dark:text-indigo-400 p-5 rounded-2xl shadow-inner shrink-0 inline-flex" aria-hidden="true">
                  {activeStep.icon}
                </div>
              ...
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200/60 dark:border-amber-800/50 p-5 rounded-2xl flex gap-4 mb-8 mt-6 shadow-sm" role="note">
                <Info className="w-6 h-6 text-amber-600 dark:text-amber-400 shrink-0" aria-hidden="true" />
              ...
              <button 
                ...
                aria-label={`${activeStep.action} — navigate to ${activeStep.title}`}
              >
                {activeStep.action}
